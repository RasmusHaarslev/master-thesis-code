...
class SchulzeService
  ...
  def strongest_path_matrix pair_wise_preference_matrix
    ...
    strongest_path_matrix = Matrix.zero(size).to_a
    for i in 0..(size - 1)
      for j in 0..(size - 1)
        unless i == j
          if array_to_work_with[i][j] > array_to_work_with[j][i]
            strongest_path_matrix[i][j] = array_to_work_with[i][j]
          end
        end
      end
    end
    ...
    for i in 0..(size - 1)
      for j in 0..(size - 1)
        unless i == j
          for k in 0..(size - 1)
            if i != k and j != k
              min                         = [strongest_path_matrix[j][i], strongest_path_matrix[i][k]].min
              strongest_path_matrix[j][k] = [strongest_path_matrix[j][k], min].max
            end
          end
        end
      end
